...
from app.tools.computer_control import CcMouseClickAtTool
...
def test_cc_mouse_click_at_success_then_denied_by_budget():
    audit = _Audit()
    scopes: dict[str, CCScope] = {"s1": _make_scope(scope_id="s1", max_actions_total=1)}
    tool = CcMouseClickAtTool(scopes=scopes, driver=FakeComputerControlDriver(), audit=audit)
...
def test_cc_mouse_click_at_invalid_range_rejected():
    tool = CcMouseClickAtTool(scopes={"s1": _make_scope(scope_id="s1")}, driver=FakeComputerControlDriver(), audit=_Audit())
