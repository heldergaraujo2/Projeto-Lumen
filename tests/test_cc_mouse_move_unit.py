...
from app.tools.computer_control import CcMouseMoveTool
...
def test_cc_mouse_move_success_consumes_budget_then_denies():
    audit = _Audit()
    scopes: dict[str, CCScope] = {"s1": _make_scope(scope_id="s1", max_actions_total=2)}
    tool = CcMouseMoveTool(scopes=scopes, driver=FakeComputerControlDriver(), audit=audit)

    r1 = tool.run(scope_id="s1", dx=10, dy=0)
    assert r1.ok is True

    r2 = tool.run(scope_id="s1", dx=10, dy=0)
    assert r2.ok is True

    r3 = tool.run(scope_id="s1", dx=10, dy=0)
    assert r3.ok is False
    assert r3.error == "denied_scope_limit_exceeded"


def test_cc_mouse_move_invalid_dx_range_rejected():
    scopes: dict[str, CCScope] = {"s1": _make_scope(scope_id="s1", max_actions_total=2)}
    tool = CcMouseMoveTool(scopes=scopes, driver=FakeComputerControlDriver(), audit=_Audit())
    r = tool.run(scope_id="s1", dx=999, dy=0)
